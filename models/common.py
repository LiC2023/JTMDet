...
class JointTransformerMambaBlockMBM(nn.Module):
    def __init__(self, dim, drop=0.1, attn_drop=0., RMSnorm=True):
        super(JointTransformerMambaBlockMBM, self).__init__()
        self.encoder1 = MambaWithNormBlock(dim, bimamba_type="v2", RMS=RMSnorm)
        self.encoder2 = Block(dim, num_heads=8, drop=drop, attn_drop=attn_drop)
        self.encoder3 = MambaWithNormBlock(dim, bimamba_type="v2", RMS=RMSnorm)
